...
interface IReservationService {
  ...
  calculateAvailableQuantity(storeId: number, itemType: string, itemId: number): Promise<number>;
}
...
describe('Inventory Reservation Service', () => {
  ...
  describe('Availability Checking', () => {
    test('should check availability for tank and item orders', async () => {
      ...
      mockReservationRepository.checkAvailability.mockResolvedValue(expectedResult);
      (reservationService.checkAvailability as jest.Mock).mockResolvedValue(expectedResult);

      const result = await reservationService.checkAvailability(request);

      expect(result.available).toBe(true);
      expect(result.details).toHaveLength(2);
      expect(result.details[0].itemType).toBe(OrderItemTypeEnum.TANK);
      expect(result.details[1].itemType).toBe(OrderItemTypeEnum.ITEM);
    });
    ...
    test('should return insufficient availability when stock is low', async () => {
      ...
      expect(result.details[0].available).toBeLessThan(result.details[0].requested);
    });
    ...
    test('should account for existing reservations in availability', async () => {
      ...
      expect(result.details[0].available).toBe(result.details[0].current - result.details[0].reserved);
    });
    ...
    test('should handle empty availability request', async () => {
      ...
      await expect(reservationService.checkAvailability(emptyRequest))
        .rejects.toThrow('At least one item must be specified for availability check');
    });
  });
  ...
  describe('Reservation Creation', () => {
    test('should create reservations for confirmed order', async () => {
      ...
      const mockReservations = [
        createMockReservation({ orderId, itemType: OrderItemTypeEnum.TANK, reservedQuantity: 2 }),
        createMockReservation({ reservationId: 2, orderId, itemType: OrderItemTypeEnum.ITEM, reservedQuantity: 1 }),
      ];
      ...
      mockReservationRepository.createReservationsForOrder.mockResolvedValue(mockReservations);
      (reservationService.createReservationsForOrder as jest.Mock).mockResolvedValue(expectedResult);

      const result = await reservationService.createReservationsForOrder(orderId);

      expect(result.success).toBe(true);
      expect(result.reservations).toHaveLength(2);
      expect(result.reservations[0].status).toBe(ReservationStatus.ACTIVE);
      expect(result.reservations[1].status).toBe(ReservationStatus.ACTIVE);
    });
    ...
    test('should fail reservation creation for insufficient inventory', async () => {
      ...
      const expectedResult: ReservationResult = {
        success: false,
        reservations: [],
        message: 'Insufficient inventory: Tank type 1 requires 5 but only 2 available',
      };
    ...
    test('should create atomic reservations (all or none)', async () => {
      ...
      const expectedResult: ReservationResult = {
        success: false,
        reservations: [],
        message: 'Reservation failed: Some items insufficient, all reservations rolled back',
      };
  ...
  describe('Reservation Fulfillment', () => {
    test('should fulfill active reservations on delivery completion', async () => {
      ...
      (reservationService.fulfillReservations as jest.Mock).mockResolvedValue(expectedResult);

      const result = await reservationService.fulfillReservations(orderId, userId);
    ...
    test('should maintain transaction atomicity during fulfillment', async () => {
      ...
      const expectedResult: FulfillmentResult = {
        success: false,
        transactions: [],
        message: 'Fulfillment failed: Transaction error, all changes rolled back',
      };
  ...
  describe('Reservation Restoration', () => {
    test('should restore reservations on order cancellation', async () => {
      ...
      const restoredReservations = [
        createMockReservation({ status: ReservationStatus.CANCELLED }),
      ];
      ...
      mockReservationRepository.restoreReservations.mockResolvedValue(restoredReservations);
      (reservationService.restoreReservations as jest.Mock).mockResolvedValue(expectedResult);

      const result = await reservationService.restoreReservations(orderId, reason);

      expect(result.success).toBe(true);
      expect(result.restoredReservations).toHaveLength(1);
      expect(result.restoredReservations[0].status).toBe(ReservationStatus.CANCELLED);
    });
    ...
    test('should restore reservations on delivery failure', async () => {
      ...
      const expectedResult: RestoreResult = {
        success: true,
        restoredReservations: [
          createMockReservation({ status: ReservationStatus.ACTIVE }), // Restored to active
        ],
    ...
    test('should handle restoration of already fulfilled reservations', async () => {
      ...
      const expectedResult: RestoreResult = {
        success: false,
        restoredReservations: [],
        message: 'Cannot restore reservations: Order already fulfilled with inventory transactions',
      };
    ...
    test('should create compensating transactions for partial deliveries', async () => {
      ...
      const expectedResult: RestoreResult = {
        ...
        message: 'Compensating transactions created for partial delivery failure',
      };
  ...
  describe('Available Quantity Calculations', () => {
    test('should calculate available quantity correctly', async () => {
      const storeId = 1;
      const itemType = OrderItemTypeEnum.TANK;
      const itemId = 1;

      (reservationService.calculateAvailableQuantity as jest.Mock).mockResolvedValue(7);

      const result = await reservationService.calculateAvailableQuantity(storeId, itemType, itemId);
    ...
    test('should return zero when no inventory available', async () => {
      ...
      (reservationService.calculateAvailableQuantity as jest.Mock).mockResolvedValue(0);
  ...
  describe('Active Reservations Management', () => {
    test('should retrieve active reservations for order', async () => {
      ...
      const activeReservations = [
        createMockReservation({ orderId, status: ReservationStatus.ACTIVE }),
        createMockReservation({ reservationId: 2, orderId, status: ReservationStatus.ACTIVE }),
      ];

      mockReservationRepository.getActiveReservations.mockResolvedValue(activeReservations);
      (reservationService.getActiveReservations as jest.Mock).mockResolvedValue(activeReservations);

      const result = await reservationService.getActiveReservations(orderId);

      expect(result).toHaveLength(2);
      expect(result[0].status).toBe(ReservationStatus.ACTIVE);
      expect(result[1].status).toBe(ReservationStatus.ACTIVE);
    });
    ...
    test('should exclude fulfilled and cancelled reservations', async () => {
      ...
      mockReservationRepository.getActiveReservations.mockResolvedValue(onlyActiveReservations);
      (reservationService.getActiveReservations as jest.Mock).mockResolvedValue(onlyActiveReservations);
  ...
  describe('Integration with Current Inventory System', () => {
    test('should check availability against current active inventory', async () => {
      ...
      expect(reservationService.checkAvailability).toHaveBeenCalledWith(request);
    });
  ...
  describe('Error Handling and Edge Cases', () => {
    test('should handle database errors during reservation creation', async () => {
      ...
      (reservationService.createReservationsForOrder as jest.Mock).mockRejectedValue(
        new Error('Database connection failed')
      );

      await expect(reservationService.createReservationsForOrder(orderId))
    ...
    test('should handle invalid order ID gracefully', async () => {
      ...
      (reservationService.createReservationsForOrder as jest.Mock).mockRejectedValue(
        new Error('Invalid order ID provided')
      );

      await expect(reservationService.createReservationsForOrder(invalidOrderId))
    ...
    test('should handle concurrent reservation conflicts', async () => {
      ...
      const expectedResult: ReservationResult = {
        success: false,
        reservations: [],
        message: 'Reservation conflict: Inventory was modified by another transaction',
      };
